...
gulp.task('sass-compile', (done) => {
	return gulp
		.src(filesForSassCompilation) // run over these files
		.pipe(plumber())
		...
		.pipe(
			gulp.dest((file) =>
				file.base.replace('/src', '/dist').replace('/scss', '/css')
			)
...
gulp.task('js-compile', (done) => {
	return (
		gulp
			.src(filesForJsCompilation) // watch these files
...
gulp.task('make-cool-shit', (done) => {
	setTimeout(() => {
		...
		console.log(' ================================================ '.red);
		console.log(
			'  Welcome to the Circus Starter template.         '.blue.bold
		);
		console.log(
			"  You're good to go.                              ".blue.bold
		);
		console.log(
			'  Make cool 💩.                                     '.blue.bold
		);
		console.log(
			'                                - Chris Silich    '.blue.dim
		);
		console.log(' ================================================ '.red);
		console.log(' ');
		done();
		...
		axios
			...
			.then((response) => {
				if (parsedPackage.version === response.data.version) {
					console.log(
						`Circus Starter template appears to be up to date.`
					);
				} else {
					console.warn(
						`Local version is ${parsedPackage.version}.`.red.bold
					);
					console.warn(
						`Remote version is ${response.data.version}.`.red.bold
					);
					console.warn(
						`Circus Starter template appears to be out of date.`.red
							.bold.inverse
					);
					console.warn(
